import json, bpy, bmesh, os, subprocess, logger # type: ignore
...
def has_properties():
    logger.debug("Entering has_properties")
    ...
    logger.info("has_properties completed successfully")
    return hasProportions, hasMaleRef, hasFemaleRef

def is_valve_biped(armature, context):
    logger.debug("Running Is_Valve_Biped")
    scene = context.scene
    ...
    simmilarityThreshold = toolBox.float_deltaAnim_similarityThreshold
    ...
    logger.info("is_valve_biped completed successfully")
    ...
    def execute(self, context):
        ...
        if hasvalvepiedarmature == False:
            for failure in failures:
                self.report({'ERROR'}, f"{failure.name} has failed to meet the current simmilarity threshold of {toolBox.float_deltaAnim_similarityThreshold} | Please either lower threshold or ensure the armature uses valve biped armature on core deformation bones.")
            return{'CANCELLED'}
...
def getallvertices_highest_vertexgroups_and_vert_location(obj):
    mesh_data = obj.data
